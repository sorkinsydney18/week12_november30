...
```{r}
covid_multi_reg <- lm(radical_intention ~ covid_burden + gender + race + age, data = covid_data)
...
us_reg <- lm(radical_intention ~ covid_burden + gender + race + age + US_ideo, data = US_covid)
...
```{r}
lm(support_BLM ~ covid_burden + gender + race + age, data = US_covid)
```
